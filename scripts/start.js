...
const DEFAULT_DEV_PORT = parseInt(DEFAULT_PORT) + 1 || 3001;
...
checkBrowsers(paths.appPath, isInteractive)
  .then(async () => {
    ...
    console.log('====================================');
    try {
      const p1 = await detect(DEFAULT_PORT, HOST);
      if (p1 !== DEFAULT_PORT) {
        throw new Error(`use ${DEFAULT_PORT} port`);
      }
      const p2 = await detect(DEFAULT_DEV_PORT, HOST);
      if (p2 !== DEFAULT_DEV_PORT) {
        throw new Error(`use ${DEFAULT_DEV_PORT} port`);
      }
      process.env.PORT = DEFAULT_PORT;
      process.env.PORT_DEV = DEFAULT_DEV_PORT;
      return DEFAULT_PORT;
    } catch (error) {
      throw error;
    }
  ...
  .then(port => {
    ...
    //
    const webConfig = configFactory('development', 'web');
    ...
    //
    const serverCompiler = compile(nodeConfig);
    //
    let watching;
    ...
    devServer.listen(parseInt(port) + 1, HOST, err => {
      if (err) {
