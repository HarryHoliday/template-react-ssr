...
module.exports = function(webpackEnv, executionEnv) {
  ...
  const isEnvProductionProfile = isEnvWeb && isEnvProduction && process.argv.includes('--profile');
  ...
  const env = getClientEnvironment(publicUrl, executionEnv);
  // add
  const clientPublicPath = isEnvDevelopment
    ? `http://${env.raw.HOST}:${parseInt(env.raw.PORT) + 1}/`
    : 'public/';
  ...
  const getStyleLoaders = (cssOptions, preProcessor) => {
    const loaders = [
      ...
      isEnvDevelopment &&
        (isEnvWeb ? require.resolve('style-loader') : require.resolve('null-loader')),
      isEnvProduction &&
        isEnvWeb && {
  ...
  return {
    ...
    // ].filter(Boolean),
    entry: isEnvNode
      ? [isEnvDevelopment && 'webpack/hot/poll?1000', paths.appServerIndexJs].filter(Boolean)
      : {
          client: [
    ...
    output: {
      ...
      // libraryTarget: isEnvNode ? 'commonjs2' : 'var',
      // The build folder.
      path: isEnvNode ? paths.appBuild : paths.appBuildPublic,
          ...
          oneOf: [
            ...
            // A missing `test` is equivalent to a match.
            {
              ...
              options: {
                ...
                publicPath: isEnvProduction ? '../../' : clientPublicPath,
              },
            },
    ...
    plugins: [
      ...
      //
      isEnvDevelopment && isEnvNode && new NodemonPlugin(),
      isEnvDevelopment && isEnvNode && new webpack.WatchIgnorePlugin([paths.appManifest]),
      isEnvNode &&
